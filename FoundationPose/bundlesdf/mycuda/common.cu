...
__device__ float determinant2x2(const Eigen::Matrix2f& m) {
...
__device__ void calculateBarycentricCoordinate2DKernel(const Eigen::Matrix<float,3,2> &triangle, const Eigen::Vector2f &p, Eigen::Vector3f &w)
{
  ...
  numerator << CA, CP;
  denominator<<AB, AC;
  w(1) = determinant2x2(numerator)/determinant2x2(denominator);
  numerator << AB, AP;
  w(2) = determinant2x2(numerator)/determinant2x2(denominator);
  w(0) = 1-w(1)-w(2);
}
