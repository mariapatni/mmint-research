...
class BopBaseReader:
  def __init__(self, base_dir, zfar=np.inf, resize=1):
    print(f"[BopBaseReader] __init__ start: base_dir={base_dir}")
    self.base_dir = base_dir
    self.resize = resize
    self.dataset_name = None
    self.color_files = sorted(glob.glob(f"{self.base_dir}/rgb/*"))
    print(f"[BopBaseReader] Found {len(self.color_files)} color files")
    if len(self.color_files)==0:
      self.color_files = sorted(glob.glob(f"{self.base_dir}/gray/*"))
      print(f"[BopBaseReader] Found {len(self.color_files)} gray files")
    self.zfar = zfar

    self.K_table = {}
    print(f"[BopBaseReader] Loading scene_camera.json from {self.base_dir}/scene_camera.json")
    ...
    print(f"[BopBaseReader] Loaded K_table with {len(self.K_table)} entries")

    if os.path.exists(f'{self.base_dir}/scene_gt.json'):
      print(f"[BopBaseReader] Loading scene_gt.json from {self.base_dir}/scene_gt.json")
      ...
      print(f"[BopBaseReader] Loaded scene_gt with {len(self.scene_gt)} entries")
    else:
      self.scene_gt = None
      print(f"[BopBaseReader] scene_gt.json not found")

    self.make_id_strs()
    print(f"[BopBaseReader] __init__ end")
  ...
  def load_symmetry_tfs(self):
    dir = os.path.abspath(os.path.join(os.path.dirname(self.base_dir), '../../models'))
    info_file = f'{dir}/models_info.json'
    print(f"[BopBaseReader] Loading models_info.json from {info_file}")
    with open(info_file,'r') as ff:
      info = json.load(ff)
    self.symmetry_tfs = {}
...
class LinemodOcclusionReader(BopBaseReader):
  def __init__(self,base_dir='/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/LINEMOD-O/lmo_test_all/test/000002', zfar=np.inf):
    print(f"[LinemodOcclusionReader] __init__ start: base_dir={base_dir}")
    ...
    print(f"[LinemodOcclusionReader] Calling load_symmetry_tfs")
    self.load_symmetry_tfs()
    print(f"[LinemodOcclusionReader] __init__ end")
...
class LinemodReader(LinemodOcclusionReader):
  def __init__(self, base_dir='/home/maria/mmint-research/FoundationPose/lm/lm_test_all/test/000001', zfar=np.inf, split=None):
    print(f"[LinemodReader] __init__ start: base_dir={base_dir}")
    super().__init__(base_dir, zfar=zfar)
    self.dataset_name = 'lm'
    if split is not None:  # train/test
      print(f"[LinemodReader] Loading split file for split={split}")
    ...
    print(f"[LinemodReader] Calling load_symmetry_tfs")
    self.load_symmetry_tfs()
    print(f"[LinemodReader] __init__ end")

  def get_gt_mesh_file(self, ob_id):
    mesh_dir = os.path.abspath(os.path.join(os.path.dirname(self.base_dir), '../../models', f'obj_{ob_id:06d}.ply'))
    print(f"[LinemodReader] get_gt_mesh_file: ob_id={ob_id}, mesh_dir={mesh_dir}")
    return mesh_dir
