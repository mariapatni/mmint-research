...
def create_output_directories(object_name):
    ...
    for camera_name in realsense_mapping.values():
        ...
        os.makedirs(os.path.join(camera_dir, "rgb"), exist_ok=True)
...
def save_frame(output_dir, device_type, serial, frame, frame_count, trial_number, debug_mode=False):
    ...
    if device_type == 'digit':
        ...
        # Save color frame
        color_filename = f"{frame_count:04d}.png"
        color_path = os.path.join(base_dir, "rgb", color_filename)
        debug_print(f"Saving RealSense rgb frame to: {color_path}", debug_mode)
        cv2.imwrite(color_path, frame['color'])
        
        # Save depth frame
        depth_filename = f"{frame_count:04d}.png"
        ...
        # Save intrinsics
        intrinsics_filename = f"cam_K.txt"
        intrinsics_path = os.path.join(base_dir, intrinsics_filename)
        
        debug_print(f"Saving RealSense intrinsics to: {intrinsics_path}", debug_mode)
        
        with open(intrinsics_path, "w") as f:
            f.write(f"{frame['intrinsics'].fx} {0.0} {frame['intrinsics'].ppx}\n")
            f.write(f"{0.0} {frame['intrinsics'].fy} {frame['intrinsics'].ppy}\n")
...
def create_combined_grid(digit_frames, realsense_frames, debug_mode=False):
    ...
    if realsense_frames:
        ...
        if "218622278343" in realsense_frames:
            ...
            # Place depth frame in bottom row (convert to 3-channel for display)
            depth_3channel = np.stack([frame_dict['depth']] * 3, axis=2)
            grid[start_y+rs_h:start_y+rs_h*2, j*rs_w:(j+1)*rs_w] = depth_3channel
