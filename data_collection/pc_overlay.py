...
def load_lidar_point_cloud(object_name):
    """
    ...
    lidar_path = os.path.join(object_path, "final_lidar_pcd.ply")
...
def load_nerfstudio_mesh(object_name):
    """
    Load the nerfstudio mesh.obj file
    """
    object_path = get_object_path(object_name)
    if not object_path:
        return None
        
    mesh_path = os.path.join(object_path, "mesh_export", "mesh.obj")
    ...
    nerf_inputs_dir = os.path.join(object_path, "nerf_data", "nerfacto")
    if not os.path.exists(nerf_inputs_dir):
        print(f"❌ Nerfstudio outputs directory not found: {nerf_inputs_dir}")
        return None
    
    # Find the latest timestamp directory
    timestamp_dirs = [d for d in os.listdir(nerf_inputs_dir) if os.path.isdir(os.path.join(nerf_inputs_dir, d))]
    if not timestamp_dirs:
        print(f"❌ No training runs found in: {nerf_inputs_dir}")
        return None
    
    # Sort by timestamp (newest first) and get the latest
    latest_timestamp = sorted(timestamp_dirs, reverse=True)[0]
    transforms_path = os.path.join(nerf_inputs_dir, latest_timestamp, "dataparser_transforms.json")
    
    if not os.path.exists(transforms_path):
        print(f"❌ Dataparser transforms file not found: {transforms_path}")
        return None
    ...
    scaled_mesh_path = os.path.join(object_path, "mesh_export", "mesh_scaled.obj")
    os.makedirs(os.path.dirname(scaled_mesh_path), exist_ok=True)
    o3d.io.write_triangle_mesh(scaled_mesh_path, mesh)
    
    return mesh

def load_nerfstudio_point_cloud(object_name):
    """
    Load the nerfstudio point cloud with the same transformations as the mesh
    """
    object_path = get_object_path(object_name)
    if not object_path:
        return None
        
    pcd_path = os.path.join(object_path, "mesh_export", "point_cloud.ply")
...
def load_foundation_point_cloud(object_name):
    """
    ...
    pcd_path = os.path.join(object_path, "FoundationPose", "debug", "scene_complete.ply")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Overlay lidar point cloud with nerfstudio mesh and point cloud')
    parser.add_argument('object', type=str, help='Name of the object to process')
    args = parser.parse_args()
    
    print(f"🔍 Lidar + Nerfstudio Overlay for object: {args.object}")
    print("=" * 50)
    
    # Load lidar point cloud
    lidar_pcd = load_lidar_point_cloud(args.object)
    if lidar_pcd is None:
        return
    
    # Load nerfstudio mesh
    nerf_mesh = load_nerfstudio_mesh(args.object)
    if nerf_mesh is None:
        return
    
    # Load nerfstudio point cloud
    nerf_pcd = load_nerfstudio_point_cloud(args.object)
    if nerf_pcd is None:
        print("⚠️  Nerfstudio point cloud not available, continuing with mesh only")
        nerf_pcd = None

    # Load foundation point cloud
    foundation_pcd = load_foundation_point_cloud(args.object)
