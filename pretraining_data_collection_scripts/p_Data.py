...
base = "."
#bpy.app.tempdir = "/Users/mariapatni/Documents/GitHub/mmint-research/temp"

data_root_folder = base + "/YCB-Slide/dataset/real/"  # Path to real folders
pose_output_folder = base + "/renders_paired/poses/"  # Where to save pose files
tactile_images_folder = base + "/renders_paired/tactile_images/"  # Where to save tactile images
stl_base_directory = base + "/YCB-slide/dataset/obj_models/"  # Path to .stl files
render_base_output_folder = base +"/renders_paired/renders/"  # Where to save rendered images
...
def generate_tactile_pose_path(object_name, dataset_number, poses):
    ...
    while pose_index < len(poses['DIGIT']):
        ...
        Q_c = Quaternion((quat_c[1], quat_c[2], quat_c[3], quat_c[0]))
        ...
        add_marker(P_c_relative, color=(1, 0, 0, 1), size=0.01)
        add_marker(Vector((0, 0, 0)), color=(0, 1, 0, 1), size=0.01)
        add_coordinate_frame(Vector((0, 0, 0)), scale=0.1)
        end = add_camera_direction_arrow(P_c_relative, Q_c, length=0.05, color=(1, 0, 0, 1))
        add_line_between_points(P_c_relative, Vector((0, 0, 0)), color=(0, 0, 1, 1))
        
        add_marker(end, color=(1, 0, 0, 1), size=0.01)
